...
import { openModal, setExpenseToDelete, setExpenseToEdit } from '@/app/slices/modalSlice';
import { useGetAllExpensesQuery } from '@/app/api/Expenses/Expenses';
import Pagination from '@/components/Pagination';

const DokonExpenses = () => {
  const [page, setPage] = useState(1);
  const [searchQuery, setSearch] = useState<string>('');
  const { data, isLoading } = useGetAllExpensesQuery({ page, search: searchQuery });
  ...
  const handleEditExpense = (expense: typeof expenseList[0]) => {
    ...
    dispatch(openModal('editExpense'));
  };

  const handleDeleteExpense = (expense: typeof expenseList[0]) => {
    dispatch(setExpenseToDelete({ id: expense.id, name: expense.name }));
    dispatch(openModal('deleteExpense'));
  };
                    ...
                    <tr key={expense.id} className='hover:bg-surface-light/50 transition-colors'>
                      ...
                      <td className='px-4 py-4'>
                        <div className='flex items-center gap-2 text-sm text-gray-300'>
                          ...
                          {new Date(expense.reminder_date).toLocaleDateString('uz-UZ')}
                        </div>
                      </td>
                      <td className='px-4 py-4'>
